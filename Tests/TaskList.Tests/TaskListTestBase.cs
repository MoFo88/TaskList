...
namespace TaskList.Tests
{
    public abstract class TaskListTestBase : AbpIntegratedTestBase<TaskListTestModule>
    {
        ...
        protected void UsingDbContext(Action<TaskListDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TaskListDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TaskListDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TaskListDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TaskListDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaskListDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TaskListDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaskListDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TaskListDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TaskListDbContext, Task<T>> func)
        {
            T result;
